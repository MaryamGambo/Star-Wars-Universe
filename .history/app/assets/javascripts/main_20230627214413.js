  ...
  if (navbarBurgers.length > 0) {
    ...
    navbarBurgers.forEach((navbarBurger) => {
      navbarBurger.addEventListener('click', () => {
        ...
        // Toggle the background color on link lists when the menu is active
        const linkLists = document.querySelectorAll('.navbar-menu .navbar-start');
        linkLists.forEach((linkList) => {
          linkList.classList.toggle('has-background-dark');
        });
      });
    });
  }
