class PeopleController < ApplicationController
  ...
  def search
    if params[:name].present? && params[:species_id].present?
      @species = Species.find(params[:species_id])
      @people = @species.people.where('people.name LIKE ?', "%#{params[:name]}%").page(page: params[:page], per_page: 15)
    elsif params[:name].present?
      @people = Person.joins(:species).where('people.name LIKE ?', "%#{params[:name]}%").page(page: params[:page], per_page: 15)
      @species = nil
    elsif params[:species_id].present?
      @species = Species.find(params[:species_id]).page(page: params[:page], per_page: 15)
      @people = @species.people
    else
      @people = Person.all.page(page: params[:page], per_page: 15)
      @species = nil
