class PeopleController < ApplicationController
  ...
  def search
    if params[:name].present? && params[:species_id].present?
      @species = Species.find(params[:species_id])
      @people = @species.people.where('people.name LIKE ?', "%#{params[:name]}%").paginate(page: params[:page], per_page: 15)
    elsif params[:name].present?
      @people = Person.joins(:species).where('people.name LIKE ?', "%#{params[:name]}%").paginate(page: params[:page], per_page: 15)
      @species = nil
    elsif params[:species_id].present?
      @species = Species.find(params[:species_id])
      @people = @species.people.paginate(page: params[:page], per_page: 15)
    else
      @people = Person.page(params[:page]).per(15)
      @species = nil
    end
